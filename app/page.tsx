...
// Ícones como componentes simples (sem biblioteca externa)
...
const App = () => {
  ...
  const getStatusIcon = (status) => {
    switch(status) {
      ...
      case 'Com Ofensor': return <XCircle />;
  ...
  const getStatusColor = (status) => {
    switch(status) {
      ...
      case 'Com Ofensor': return 'bg-red-500';
  ...
  const removerItem = (tipo, index) => {
    if (confirm('Tem certeza que deseja remover este item?')) {
      if (tipo === 'solicitante') {
        ...
        const alinhadoresNovos = alinhadores.filter((_, i) => i !== index);
        setAlinhadores(alinhadoresNovos);
  ...
  const salvarEdicao = () => {
    if (editandoItem.valor.trim()) {
      if (editandoItem.tipo === 'solicitante') {
        ...
        const alinhadoresAntigo = novos[editandoItem.index];
        novos[editandoItem.index] = editandoItem.valor.trim();
        setAlinhadores(novos);
        
        const novaCor = coresAlinhadores[alinhadoresAntigo];
        const novasCores = {...coresAlinhadores};
        delete novasCores[alinhadoresAntigo];
